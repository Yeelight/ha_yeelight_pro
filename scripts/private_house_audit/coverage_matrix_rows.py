"""CSV row shaping for private-house coverage matrix exports."""
...
FIELDNAMES = [
    ...
    "control_actual",
    "control_missing",
    "strict_control_expected",
    "strict_control_actual",
    "strict_control_missing",
    "strict_control_platforms",
    "strict_control_absence_reason",
    "sensor_status",
    "sensor_expected",
    "sensor_actual",
    "sensor_missing",
...
def coverage_matrix_row(item: Any) -> dict[str, Any]:
    """Return one CSV row from a classified device or topology item."""
    ...
    return {
        ...
        "strict_control_expected": _int_value(control_view.get("expected")),
        "strict_control_actual": _int_value(control_view.get("actual")),
        "strict_control_missing": _int_value(control_view.get("missing")),
        "strict_control_platforms": _inline_mapping(
            strict_control_platforms(actual_platforms)
        ),
        "strict_control_absence_reason": control_absence_reason(item),
        ...
        "control_platforms": _platform_family(
            actual_platforms,
            STRICT_CONTROL_PLATFORMS,
        ),
        "primary_control_platforms": _platform_family(
            actual_platforms,
            PRIMARY_CONTROL_PLATFORMS,
        ),
        "sensor_platforms": _platform_family(
            actual_platforms,
            ("binary_sensor", "sensor"),
        ),
        "diagnostic_platforms": _diagnostic_platforms(
            actual_platforms,
            actual_roles,
        ),
        ...
        "source_raw_property_count": _int_value(
            source_evidence.get("raw_property_count")
        ),
        "source_raw_property_keys": _sequence_text(
            source_evidence.get("raw_property_keys")
        ),
        ...
        "source_subdevice_property_keys": _sequence_text(
            source_evidence.get("subdevice_property_keys")
        ),
        ...
        "instance_state_keys_count": _int_value(
            item.get("instance_state_keys_count")
        ),
        "projected_component_count": _int_value(
            item.get("projected_component_count")
        ),
        "schema_gap_reason": str(item.get("schema_gap_reason") or ""),
        "low_coverage_reasons": _sequence_text(item.get("low_coverage_reasons")),
        "param_keys": _sequence_text(item.get("param_keys")),
        "projected_component_ids": _sequence_text(
            item.get("projected_component_ids")
        ),
        "expected_samples": _entity_samples_text(item.get("expected_samples")),
        "missing_samples": _entity_samples_text(item.get("missing_samples")),
...
def _sequence_text(value: Any) -> str:
    """Return pipe-delimited sequence values."""
    return "|".join(str(item) for item in _sequence(value))


def _coverage_percent(*, expected_total: int, missing_total: int) -> str:
...
def _diagnostic_platforms(
    ...
    if not diagnostic_total:
        return ""
    platforms_text = _platform_family(platforms, ("binary_sensor", "sensor"))
    return f"diagnostic={diagnostic_total};platforms={platforms_text}"
...
__all__ = ["FIELDNAMES", "coverage_matrix_row"]
