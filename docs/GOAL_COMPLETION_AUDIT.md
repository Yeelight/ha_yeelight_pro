...
本文按用户目标记录 `ha_yeelight_pro` 当前实现和验证入口。发布审查以当前命令输出和本地 HA 安装态为准。
...
ruff check .
python3 -m compileall -q custom_components/yeelight_pro scripts
python3 scripts/hacs_preflight.py
