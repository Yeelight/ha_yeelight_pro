...
HACS PR 前都必须重新运行这些入口并复核脱敏输出。

## HACS 自定义仓库安装

PR 合并前，HACS 用户可以把本仓库添加为 custom repository：

1. 打开 Home Assistant，进入 HACS。
2. 点击右上角三个点菜单。
3. 选择 `Custom repositories`。
4. Repository 填写 `https://github.com/Yeelight/ha_yeelight_pro`。
5. Type 选择 `Integration`。
6. 点击 `Add`。
7. 在 HACS 搜索 `Yeelight Pro`，打开后点击 `Download`。
8. 重启 Home Assistant。
9. 进入“设置 -> 设备与服务 -> 添加集成 -> Yeelight Pro”。

如果重启后看不到集成，刷新浏览器缓存，并确认 Home Assistant 配置目录下存在
...
本地 HA 开发安装必须使用 `scripts/sync_local_ha_runtime.py`，不要直接 `cp -R`
...
- 诊断导出：确认存在 `spec_correction`、`spec_runtime_inventory`、
  `entity_candidates`、`device_import_filter_preview` 和
  `entity_import_filter_preview` 聚合；确认 token、house ID、device ID、MAC、私有域名、product model id、component/property/event/action 明细、raw payload 和 raw filter rule 不出现在 JSON 中。
- 设备 picker：云端配置流在选择家庭后只读加载真实设备列表，默认全选，取消勾选后保存为后续设备来源实体的导入过滤规则；云端 entry 的 options 也可重新打开真实设备 picker 调整选择。设备列表加载失败时，setup 可继续创建 entry 且过滤关闭，options 不覆盖现有过滤规则。
