...
- 云端模式、私有部署模式和局域网网关模式配置入口。
- 标准 Home Assistant 重认证错误传播。
- 设备注册表拓扑同步。
- 易来 IoT 品类到 Home Assistant 平台的投影矩阵。
- 可配置轮询间隔、调试模式、实时更新、局域网网关控制和未知能力隐藏策略。
- APP 扫码登录、多区域 API 域名、显式 opt-in WebSocket runtime、局域网网关 TCP runtime、setup/options 真实设备 picker、房屋级数据分析诊断传感器和非破坏性 registry cleanup。
...
- 局域网网关控制位于局域网模式 entry 和 `local_gateway_control` 选项边界内；host 为空时执行一次局域网 UDP discovery fallback 后建立 TCP 会话。
