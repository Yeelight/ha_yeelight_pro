...
`event`、`scene`、`button`、`select`、`number` 和 `vacuum` 是 Home Assistant 实体平台或投影目标，不是易来 IoT 设备品类。`vacuum` 为可选实验平台，默认不启用。

物模型注册表边界见 [docs/IOT_SPEC_REGISTRY.md](docs/IOT_SPEC_REGISTRY.md)；ha_xiaomi_home 架构对比和已借鉴实现见 [docs/HA_XIAOMI_HOME_GAP_REVIEW.md](docs/HA_XIAOMI_HOME_GAP_REVIEW.md)。
...
当前仓库已包含 HACS 元数据和发布包校验入口，按常规发布审查流程提交。
...
- 云端事件通知使用显式 `live_updates` WebSocket runtime；轮询仍作为默认全量状态刷新路径。
- 本地网关控制使用显式 `local_gateway_control` 选项；启用后若未配置 host，runtime 会按局域网协议执行一次 UDP 发现，再建立 TCP 网关会话。
