    ...
    for prop in subdevice.get("properties") or []:
        ...
        properties.append(
            ...
            or PropertyModel(
                ...
                value_range=_value_range(prop.get("valueRange")),
                value_list=_value_list(prop.get("valueList"), string_value),
            )
        )
    return properties


def _property_from_openapi(
    prop_id: str,
    ...
    *,
    ...
    value_range = _value_range(prop.get("valueRange"))
    value_list = _value_list(prop.get("valueList"), string_value)
    template = _template_property(prop_id, build_property)
    ...
    return PropertyModel(
        prop_id=prop_id,
        name=desc or (template.name if template is not None else None),
        desc=desc,
        kind=_property_kind(prop, string_value)
        or (template.kind if template is not None else None),
        property_type=template.property_type if template is not None else "apply",
        format=prop_format or (template.format if template is not None else None),
        unit=unit or (template.unit if template is not None else None),
        zoom=normalize_zoom(prop.get("zoom")),
        scale=normalize_scale(prop.get("scale")),
        access=access or (template.access if template is not None else None),
        value_range=value_range
        or (template.value_range if template is not None else None),
        value_list=value_list or (template.value_list if template is not None else []),
    )
