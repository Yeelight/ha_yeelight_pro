...
from .runtime_bridge_properties import (
...
class RuntimePayloadBridge(RuntimePropertyApplyMixin):
    ...
    def resolve_event_payloads(
        self,
        payloads: Sequence[Mapping[str, Any]],
    ) -> list[dict[str, Any]]:
        """Resolve event source aliases before event dedupe and dispatch."""
        return [self._resolve_event_payload(payload) for payload in payloads]

    def _resolve_event_payload(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        """Return an event payload whose source id matches loaded topology."""
        result = dict(payload)
        result.pop(ATTR_NODE_ID_CANDIDATES, None)
        source_id = coerce_device_id(payload.get(ATTR_SOURCE_DEVICE_ID))
        if source_id is None:
            return result
        candidates = runtime_event_node_id_candidates(payload, source_id)
        if not candidates:
            return result
        resolved = self._resolve_update_node(
            RuntimePropertyUpdate(
                node_id=source_id,
                node_type=runtime_event_node_type(payload),
                params={},
                node_id_candidates=candidates,
            )
        )
        if resolved.node_id != source_id:
            result[ATTR_SOURCE_DEVICE_ID] = str(resolved.node_id)
        return result
