...
def test_classify_device_marks_writable_model_without_control_as_source_limited() -> None:
    ...
    assert result["status"] == STATUS_SOURCE_DATA_LIMITED
    assert result["action"] == ACTION_INVESTIGATE_SOURCE_DATA
    assert result["reason"] == "writable_model_properties_without_primary_control_projection"
...
def test_classified_row_marks_control_bucket_needs_review_for_writable_model() -> None:
    ...
    row = classified_device_row(
        ...
        },
        {
            ...
            "reason": "writable_model_properties_without_primary_control_projection",
        },
    )
    ...
    assert row["coverage_view"]["control"] == {
        ...
        "missing": 0,
        "status": "needs_review",
        "attention": "model_has_writable_properties_but_no_primary_control",
    }


def test_classified_row_does_not_mark_scene_panel_control_attention() -> None:
    """事件输入设备完整覆盖事件实体时，control bucket 应显示不期望主控制。"""
    row = classified_device_row(
        {
            "category": "scene_panel",
            "actual_total": 5,
