...
async def test_options_flow_private_entry_shows_push_url(mock_config_entry) -> None:
    ...
    assert defaults[CONF_PRIVATE_PUSH_PROXY] == ""
...
async def test_options_flow_private_push_change_updates_entry_data(
    ...
    assert update.kwargs["data"][CONF_PRIVATE_PUSH_PROXY] == ""


@pytest.mark.asyncio
async def test_options_flow_private_push_proxy_change_updates_entry_data(
    mock_config_entry,
    mock_hass,
) -> None:
    """Saving a private push proxy updates config-entry data and asks for reload."""
    mock_config_entry.data.update({
        CONF_CONNECTION_MODE: CONNECTION_MODE_PRIVATE,
        CONF_PRIVATE_DOMAIN: "http://api-test.yeedev.com",
...
async def test_options_flow_non_private_entries_hide_push_url(
    ...
    assert CONF_PRIVATE_PUSH_PROXY not in defaults
